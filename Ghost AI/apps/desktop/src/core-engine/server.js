...
const logger = new Logger_1.Logger('EngineServer');
...
class EngineServer {
    ...
    eventBus;
    contextService;
    constructor(eventBus, contextService) {
    ...
    setupRoutes() {
        ...
        const possiblePaths = [
            path_1.default.join(__dirname, '../public/spy'),
            path_1.default.join(__dirname, 'public/spy'),
            path_1.default.join(process.cwd(), 'apps/core-engine/public/spy'),
            path_1.default.join(process.cwd(), 'public/spy')
        ];
        const publicSpyPath = possiblePaths.find(p => fs_1.default.existsSync(p)) || possiblePaths[0];
        this.app.use('/spy', express_1.default.static(publicSpyPath));
        this.app.get('/spy', (_req, res) => {
            const indexPath = path_1.default.join(publicSpyPath, 'index.html');
            ...
            if (this.contextService) {
                const initialState = {
                    ...
                    payload: {
                        ...
                        history: this.contextService.historyRing?.toArray() || []
                    }
                };
        ...
        this.app.post('/api/audio-chunk', express_1.default.raw({ type: 'application/octet-stream', limit: '10mb' }), (req, res) => {
